...
int main(int argc, char* argv[]) {
  ...
  try {
    ...
    https.request(request,
      [&response, &bodyString, &firstChunk]
      (const pockethttp::Response& res, const std::vector<uint8_t>& chunk) {
        if (firstChunk) {
          response = res;
          firstChunk = false;
    ...
    std::cout << response.version << " " << response.status << " "
              << response.statusText << std::endl;

    std::cout << response.headers.dump() << std::endl;
